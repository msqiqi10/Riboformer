...
def read_wig(filename):
    ...
    with open(filename + '_f.wig', 'r') as read_file:
        ...
        for j in range(1, len(lines)):
            ...
            if line and line[0] == 'f':
                chrom_all.append(line.split(' ')[1].split('=')[1].replace('chr', ''))
                if j > 2:
                    wig1[chrom_all[-2]] = np.array(read)
                read = []
            else:
                try:
                    read.append(float(line))
                except ValueError:
                    continue
    if chrom_all:
    ...
    wig_total = {}
    for c in wig1.keys():
        arrays_to_stack = [np.zeros(wig1[c].shape), wig1[c], wig2[c]]
        wig_total[c] = np.vstack(arrays_to_stack).transpose()
    return wig_total
...
def generate_training(my_data2, seq, Dwig1, Dwig2, wsize, Ctable, P_site: int = 14, thres: int = 25):
    '''
    input
    ...
    for i, data in enumerate(my_data2):
        ...
        if data[2] == 1:
            ...
            if data[1] - data[0] > gene_len:
                ...
                RD2 = RD2[::-1]
                seq_t = seq[int(data[0]) - 1 - n : int(data[1]) + n]
                seq_t = ''.join(complement.get(base, base) or base for base in reversed(seq_t))
...
def main():
    ...
    parser.add_argument('-d', '--data_dir', default = '/home/zzz0054/chen_data/Riboformer/datasets/AA/', 
                        help = 'data folder name')
    parser.add_argument('-r', '--reference', default = 'AuxinC0FootprintR4_DNA', 
                        help = 'reference dataset name')
    parser.add_argument('-p', '--psite', default = 14, type = int,
                        help = 'offset for p site position')
    parser.add_argument('-t', '--target', default = 'AuxinA0FootprintR4_DNA', 
    ...
    if gff_file:
        for key in seq_dict.keys():
            if key != 'Mito':
                ...
                x_c, y_c, z_c = generate_training(gff_data, seq_dict[key], Dwig1[key], Dwig2[key], args.wsize, Ctable, int(args.psite), int(args.threshold))
