...
def convert_bam_to_wig(bam_file, output_prefix, p_site_offset=14):
    ...
    with open(f"{output_prefix}_f.wig", "w") as f_out:
        ...
        for chrom in chromosomes:
            ...
            counts = forward_counts.get(chrom, {})
            if counts:
                min_pos = 0
                max_pos = chrom_length - 1
                f_out.write(f"fixedStep chrom={chrom} start={min_pos + 1} step=1\n")
                ...
                max_pos = chrom_length - 1
                f_out.write(f"fixedStep chrom={chrom} start={min_pos + 1} step=1\n")
                for _ in range(min_pos, max_pos + 1):
                    f_out.write("0\n")
    ...
    with open(f"{output_prefix}_r.wig", "w") as r_out:
        ...
        for chrom in chromosomes:
            chrom_length = chr_lengths[chrom]
            counts = reverse_counts.get(chrom, {})
            if counts:
                min_pos = 0
                max_pos = chrom_length - 1
                r_out.write(f"fixedStep chrom={chrom} start={min_pos + 1} step=1\n")
                for pos in range(min_pos, max_pos + 1):
                    count = counts.get(pos, 0)
                    # Use negative counts for reverse strand
                    r_out.write(f"{-count}\n")
            else:
                # Write zeros if no counts for this chromosome
                min_pos = 0
